...
import { DeploymentModel, PodDisruptionBudgetModel  } from '../models';
import {
  DeploymentKind,
  K8sKind,
...
export const PodDisruptionBudgetField: React.FC<PodDisruptionBudgetFieldProps> = ({ pdbObj:obj }) => {
  const { t } = useTranslation();
  return (
    <dl className="co-m-pane__details">
      <DetailsItem label={t('public~PodDisruptionBudgets')} obj={obj} path="">
      {/* <ResourceLink
...
export const DeploymentDetailsList: React.FC<DeploymentDetailsListProps> = ({ deployment, pdbObj }) => {
  const { t } = useTranslation();
  return (
    <dl className="co-m-pane__details">
      ...
      // <PodDisruptionBudgetField obj={pdbObj} />
    </dl>
  );
};
...
const DeploymentDetails: React.FC<DeploymentDetailsProps> = ({ obj: deployment }) => {
  ...
  const filterPdbbySelector = resources.filter(f => _.isEqual(f.spec.selector.matchLabels, deployment.spec.selector.matchLabels))
  console.log("filterPdbbySelector", filterPdbbySelector)
      ...
      <div className="co-m-pane__body">
        ...
        <div className="co-m-pane__body-group">
          <div className="row">
            ...
            <div className="col-sm-6">
              <DeploymentDetailsList deployment={deployment} pdbObj={filterPdbbySelector} />
            </div>
          </div>
        </div>
      </div>
